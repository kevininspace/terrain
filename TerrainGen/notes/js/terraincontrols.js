...
var primH = zero(generateGoodMesh(10));
...
physDiv.append("button")
    .text("Generate random heightmap")
    .on("click", function () {
        physH = generateCoast({npts:10, extent:defaultExtent});
        physDraw();
    });
...
function newCityRender(h) {
    h = h || generateCoast({npts:10, extent: defaultExtent});
    return {
