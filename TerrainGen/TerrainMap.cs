...
namespace TerrainGen
{
    internal class TerrainMap
    {
        ...
        private double runif(int lo, int hi)
        {
        ...
        private double[] randomVector(int scale)
        {
        ...
        private List<Site> neighbours(Mesh mesh, int i)
        {
        ...
        private object downhill(HeightMap h)
        {
            if (h.downhill == null)
            {
                return h.downhill;
        ...
        private Site downfrom(HeightMap h, int i)
        {
            if (isedge(h.mesh, i))
            {
                return null; // -2;
        ...
        private void visualizePoints(List<Point> pts)
        {
            SvgCircle spCircle;
        ...
        private void makeD3Path(List<PointF> path)
        {
        ...
        private void drawPoints(List<Point> pts)
        {
            visualizePoints(pts);
        ...
        private HeightMap generateCoast(MapParams mapParams)
        {
        ...
        public void doMap()
        {
            ...
            SvgUnit width = param.extent.Width; //svg.Width;
            svg.Height = width * param.extent.Height / param.extent.Width;
            svg.ViewBox = new SvgViewBox(
                -1 * param.extent.Width / 2,
                -1 * param.extent.Height / 2,
                1 * param.extent.Width,
                1 * param.extent.Height);
            //svg.selectAll().remove();
            RenderObject render = new RenderObject();
            render.h = generateCoast(param);
            placeCities();
            drawMap(svg, render);
        }
